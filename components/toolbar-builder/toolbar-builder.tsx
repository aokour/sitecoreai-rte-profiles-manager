...
import { Eye, Code, Layers, AlertCircle, AlertTriangle, CheckCircle, Paintbrush, Settings, AlertCircle as InfoIcon } from "lucide-react";
import { Switch } from "@/components/ui/switch";
import { validateProfileConfig, type EditorProfileConfig, type ToolbarGroup, type ValidationResult, type StyleDefinition } from "@/types";
...
export function ToolbarBuilder({
  ...
  const parseConfigToItems = useCallback((config: EditorProfileConfig | undefined): ToolbarConfigItem[] => {
    ...
    return config.toolbar.items.map((item) => {
      ...
      if (typeof item === "object" && item !== null) {
        return { id: generateId(), type: "group" as const, group: item as ToolbarGroup };
      }
      return { id: generateId(), type: "item" as const, itemId: item as string };
    });
  }, []);

  const [items, setItems] = useState<ToolbarConfigItem[]>(() => parseConfigToItems(initialConfig));
  const [styles, setStyles] = useState<StyleDefinition[]>(() => initialConfig?.style || []);
  const [disableContentWrap, setDisableContentWrap] = useState<boolean>(() => initialConfig?.disableContentWrap || false);
  ...
  const itemsToConfig = useCallback((builderItems: ToolbarConfigItem[], currentStyles: StyleDefinition[], contentWrapDisabled: boolean): EditorProfileConfig => {
    const toolbarItems = builderItems.map((item) => {
  ...
  const updateConfig = useCallback((newItems: ToolbarConfigItem[], newStyles?: StyleDefinition[], newDisableContentWrap?: boolean) => {
    setItems(newItems);
    const currentStyles = newStyles !== undefined ? newStyles : styles;
    const currentDisableContentWrap = newDisableContentWrap !== undefined ? newDisableContentWrap : disableContentWrap;
    onConfigChange(itemsToConfig(newItems, currentStyles, currentDisableContentWrap));
  }, [onConfigChange, itemsToConfig, styles, disableContentWrap]);

  // Update styles
  const handleStylesChange = useCallback((newStyles: StyleDefinition[]) => {
    setStyles(newStyles);
  ...
  const handleDisableContentWrapChange = useCallback((value: boolean) => {
  ...
  const currentConfig = useMemo(() => itemsToConfig(items, styles, disableContentWrap), [items, styles, disableContentWrap, itemsToConfig]);
            ...
            <div className="space-y-0">
              ...
              <Card className="rounded-b-none border-b-0">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                  <div>
                    <CardTitle className="text-sm">Toolbar Configuration</CardTitle>
                    <CardDescription>
          ...
          <TabsContent value="json" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Edit the toolbar configuration directly in JSON format. Only valid toolbar items will be accepted.
                </CardDescription>
              </CardHeader>
                  ...
                  onChange={(e) => {
                    ...
                    if (validation.valid) {
                      try {
                        ...
                        setDisableContentWrap(parsed.disableContentWrap === true);
                    ...
                    <div className="p-3 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900">
                      ...
                      <ul className="text-sm text-red-600 dark:text-red-400 space-y-1 list-disc list-inside">
                        ...
                        {jsonValidation.errors.length > 5 && (
                          <li className="text-red-500">...and {jsonValidation.errors.length - 5} more errors</li>
                        )}
                      </ul>
                    </div>
                  )}
      ...
      <Dialog open={stylesModalOpen} onOpenChange={setStylesModalOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Define custom styles that authors can apply to content in the RTE editor.
            </DialogDescription>
          </DialogHeader>
          <StyleEditor
            styles={styles}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label htmlFor="disable-content-wrap" className="text-base font-medium">
                    Disable Content Wrapper
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    Disable the automatic <code className="px-1 py-0.5 bg-muted rounded text-xs">ck-content</code> wrapper around RTE output
                  </p>
                </div>
                <Switch
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <p className="text-amber-700 dark:text-amber-500">
                        When disabled, the RTE will output content without the automatic <code className="px-1 py-0.5 bg-amber-100 dark:bg-amber-900/50 rounded text-xs">ck-content</code> wrapper. You must ensure:
                      </p>
                      <ul className="list-disc list-inside text-amber-700 dark:text-amber-500 space-y-1 ml-2">
                        <li>Your HTML markup uses the <code className="px-1 py-0.5 bg-amber-100 dark:bg-amber-900/50 rounded text-xs">ck-content</code> class in an ancestor element, OR</li>
                        <li>Provide equivalent CSS in your site&apos;s stylesheet that replaces all <code className="px-1 py-0.5 bg-amber-100 dark:bg-amber-900/50 rounded text-xs">.ck-content</code> rules</li>
                      </ul>
                      <p className="text-amber-700 dark:text-amber-500 mt-2">
                        Without proper styling, features like images, block quotes, tables, and typography may not render correctly.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900">
                  <div className="flex gap-3">
                    ...
                    <div className="space-y-2 text-sm text-blue-700 dark:text-blue-400">
                      <p>
                        <strong>Default behavior:</strong> RTE output is wrapped in <code className="px-1 py-0.5 bg-blue-100 dark:bg-blue-900/50 rounded text-xs">&lt;div class=&quot;ck-content&quot;&gt;...&lt;/div&gt;</code>
                      </p>
                      <p>
                        This ensures content styles (images, block quotes, tables, typography) are properly scoped and rendered consistently between the editor and published site.
                      </p>
                    </div>
                  </div>
                </div>
